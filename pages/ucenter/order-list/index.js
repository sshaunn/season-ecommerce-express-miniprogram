...
Page({
    ...
    getOrderList() {
        ...
        util.request(api.OrderList, {
            ...
            if (res.errno === 0) {
                ...
                that.setData({
                    allCount: count,
                    allOrderList: that.data.allOrderList.concat(res.data.orders),
                    allPage: res.data.currentPage,
                    orderList: that.data.allOrderList.concat(res.data.orders)
                });
                let hasOrderData = that.data.allOrderList.concat(res.data.orders);
                if (count == 0) {
                    that.setData({
                        hasOrder: 1
                    });
    ...
    onReachBottom: function () {
        let that = this;
        if (that.data.allCount < that.data.size) {
            that.setData({
                showTips: 1
            });
            return false;
        }
        // console.log('current page', that.data.allPage);
        that.setData({
            'allPage': parseInt(that.data.allPage) + 1
        });
        that.getOrderList();
    },
